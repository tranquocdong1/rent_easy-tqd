...
import { Building2, ArrowLeft, Save, AlertCircle, ChevronDown, Check } from "lucide-react";
...
export default function EditPropertyPage() {
    ...
    const fetchProperty = async () => {
      try {
        ...
        if (error.response?.status === 404) {
          ...
          setGlobalError("Lỗi khi tải thông tin Bất động sản");
        }
      } finally {
        setIsLoading(false);
      }
  ...
  if (notFound) {
    return (
      <div className="bg-white rounded-2xl p-12 border border-slate-200/90 shadow-2xs max-w-2xl mx-auto text-center space-y-4 my-8">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Không tìm thấy Bất động sản</h2>
        <p className="text-sm text-slate-500 font-medium max-w-md mx-auto">
          Bất động sản không tồn tại hoặc bạn không có quyền truy cập thông tin này.
        </p>
        <Button
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium mt-0.5">
              Cập nhật thông tin chi tiết nhà trọ và trạng thái vận hành
            </p>
          </div>
        </div>
      </div>
